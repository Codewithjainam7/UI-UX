...
export function Layout({ children, activeTab, setActiveTab }: LayoutProps) {
  const topNav: NavItem[] = [
    { id: 'inbox', icon: <Inbox size={20} />, label: 'Inbox', badge: 6 },
    { id: 'analytics', icon: <BarChart2 size={20} />, label: 'Analytics' },
    { id: 'automation', icon: <Sparkles size={20} />, label: 'Automation' },
    { id: 'templates', icon: <FileText size={20} />, label: 'Templates' },
    { id: 'components', icon: <FileText size={20} />, label: 'Component Library' },
  ];
  ...
  return (
    <div className="flex h-screen w-full bg-background relative overflow-hidden font-sans">
      ...
      {/* Sidebar */}
      <nav className="relative z-40 hidden md:flex flex-col w-64 h-full border-r border-white/10 bg-black/20 backdrop-blur-[40px] p-4 flex-shrink-0">
        ...
        <button className="w-full bg-gradient-to-r from-primary-container to-indigo-500 text-white text-sm font-semibold uppercase tracking-wider rounded-lg py-3 mb-6 flex items-center justify-center gap-2 shadow-[0_4px_14px_rgba(124,58,237,0.39)] hover:opacity-90 transition-opacity">
        ...
        <div className="flex-1 flex flex-col gap-1">
          ...
          <button
            ...
            className={cn(
              "flex items-center gap-3 px-4 py-3 rounded-lg transition-all mt-4",
              activeTab === 'settings'
                ? "bg-white/10 text-white border-l-2 border-violet-500"
                : "text-slate-400 hover:text-white hover:bg-white/5 active:scale-[0.97]"
            )}
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative z-10 w-full min-w-0">
        {/* Header */}
        <header className="flex-shrink-0 h-16 flex justify-between items-center px-6 border-b border-white/10 bg-slate-950/40 backdrop-blur-[40px] z-50">
          <div className="flex items-center gap-4 w-full max-w-md">
            <Menu size={24} className="text-on-surface-variant md:hidden cursor-pointer" />
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="p-2 text-slate-400 hover:text-white hover:bg-white/5 transition-all rounded-full hidden sm:block">
              <HelpCircle size={20} />
            </button>
